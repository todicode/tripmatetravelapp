...
12. `POST /auth/change-password` requires a bearer access token and accepts `currentPassword` and `newPassword`. The user is taken from the authenticated session. It verifies the current password, rejects unchanged passwords, hashes the new password, and invalidates all device bindings and refresh tokens in one transaction. Returns `204`; log in again afterwards. New passwords require 8–128 characters and at most 72 UTF-8 bytes (BCrypt limit). Errors include `CURRENT_PASSWORD_INCORRECT`, `PASSWORD_UNCHANGED`, `PASSWORD_TOO_LONG`, and `PASSWORD_CHANGE_UNAVAILABLE` for Google accounts without a known local password. Attempts use the existing auth rate limit.
